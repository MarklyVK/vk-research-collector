...
| `group_posts` | PK bigint; FK group CASCADE; UQ `(vk_owner_id,vk_post_id)` | published/edited, text, counters, signer, hash; indexes group+date, signer | Upsert одной версии; максимум 2,45 млн при лимите 200 |
...
| `collection_errors` | PK bigint; FK run/job CASCADE | token fingerprint, endpoint, category, VK/HTTP code, attempt, sanitized message, time | Append-only диагностическая история с retention |
