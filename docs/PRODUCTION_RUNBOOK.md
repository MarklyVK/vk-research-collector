...
Сетевые ошибки VK повторяются внутри HTTP-запроса только с короткими задержками 1/3/10
секунд; дальнейший backoff сохраняется в PostgreSQL и не удерживает один процесс часами.
При следующем запуске control также автоматически возвращает остановленный worker в
healthy-состояние после прерванного maintenance.
Pilot не ждёт длинный durable-backoff внутри GitHub Actions: незавершённое задание
остаётся в PostgreSQL, worker возвращается в healthy, а следующий hourly-control
продолжает безопасный проход.
Пользователь с transient-сбоем получает cooldown на сутки и не блокирует каждую новую
pilot cohort; само задание сохраняет обычный durable retry/backoff.
