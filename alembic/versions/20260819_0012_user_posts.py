...
def upgrade() -> None:
    ...
    # 1. Добавление демографических полей в vk_users (если отсутствуют)
    user_columns = {column["name"] for column in inspector.get_columns("vk_users")}
    if "sex" not in user_columns:
        op.add_column("vk_users", sa.Column("sex", sa.Integer(), nullable=True))
    if "bdate" not in user_columns:
    ...
    if "relation" not in user_columns:
    ...
    if "user_post_attachments" not in existing_tables:
        op.create_table(
            ...
            sa.UniqueConstraint(
                "post_id", "position", name="uq_user_post_attachments_post_position"
            ),
        )
        op.create_index(
            "ix_user_post_attachments_type", "user_post_attachments", ["attachment_type"]
        )

    # 4. Создание таблицы user_post_collection_states
    if "user_post_collection_states" not in existing_tables:
        op.create_table(
